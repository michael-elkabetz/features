...
features serve -p 8080

# Live mode: trigger re-analysis from the UI without leaving the browser
features serve --live
```
...
Starts a local web viewer at `http://localhost:4747` so you can browse the knowledge generated by `features init`. Press `Ctrl-C` to stop.

| Option | Description |
|--------|-------------|
| `-p, --port <port>` | Port to listen on (default: 4747) |
| `--live` | Enable live mode: trigger and watch re-analysis from the browser UI |
| `-m, --model <model>` | Model used for live-mode analysis (default: `sonnet`) |
